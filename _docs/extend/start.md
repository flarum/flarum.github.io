...
Don't fret if you get stuck – there are plenty of people on the [Community Forum](http://discuss.flarum.org/t/extensibility) and [Gitter Chat](https://gitter.im/flarum/flarum) who are more than willing to help. Maybe one day, when you're a Level 99 Black Belt Flarum Extension Zen Master, you can pass on your knowledge too!
...
Now create a file at `js/forum/src/main.js`. This is like the JavaScript equivalent of `bootstrap.php` – its contents is executed as the JavaScript application boots up. This is where we will make our changes to the UI. For now, though, let's just alert a friendly greeting:
...
$events->listen(ConfigureClientView::class, function (ConfigureClientView $event) {
    if ($event->isForum()) {
        ...
        $event->addBootstrapper('acme/hello-world/bootstrap');
    }
});
```
